...
Assessing the performance of text generation tasks is challenging, especially for automatic revisions of scientific content.
In this context, it is crucial to ensure that the revision does not alter the original meaning or introduce incorrect or misleading information.
...
We used three manuscripts of our own authorship (CCC, PhenoPLIER, and Manubot-AI; see below), which allowed us to assess changes in the original meaning and determine whether revisions retained important details.
During the prompt engineering phase (see below), we also employed a unit testing framework to ensure that the revisions produced by our prompts met a minimum set of quality standards.
Finally, by incorporating two external manuscripts (BioChatter and Epistasis), we utilized the LLM-as-a-Judge technique [@arxiv:2306.05685], where we asked an LLM to evaluate the quality of the revisions produced by another LLM.
...
For the evaluation of our tool, we conducted both manual assessments performed by humans and automatic assessments executed by an LLM.
For the human assessments, we utilized three of our own manuscripts (Table @tbl:manuscripts): the Clustermatch Correlation Coefficient (CCC) [@doi:10.1101/2022.06.15.496326], PhenoPLIER [@doi:10.1038/s41467-023-41057-4], and Manubot-AI (this manuscript).
CCC introduces a novel correlation coefficient applied to transcriptomic data, while PhenoPLIER is a framework that encompasses three distinct methods used in genetic studies.
CCC is situated within the field of computational biology, whereas PhenoPLIER is related to genomic medicine.
CCC discusses a computational method tailored to a specific data type (correlation to gene expression).
In contrast, PhenoPLIER presents a framework that integrates three different approaches—regression, clustering, and drug-disease prediction—leveraging data from genome-wide and transcription-wide association studies (GWAS and TWAS), gene expression, and transcriptional responses to small molecule perturbations.
Consequently, CCC has a more straightforward structure, whereas PhenoPLIER is a more intricate manuscript, featuring additional figures and tables, along with a Methods section that includes equations.
The third manuscript, Manubot-AI, possesses a significantly simpler structure and was written and revised using our tool prior to submission, showcasing a practical AI-based revision use case.
For the automatic assessments, we included two external manuscripts (identified as BioChatter and Epistasis in Table @tbl:manuscripts).


#### Evaluation using human assessments

The paragraph is well-written and clear.
However, there are minor adjustments that could enhance its clarity and consistency:
...
This action added the 'ai-revision' workflow to the 'Actions' tab of each repository.
...
Changes made:
- Added a colon after "manuscripts" for better list introduction.
- Changed "used" to "utilized" for a slightly more formal tone.
- Minor adjustments for flow and consistency.
...
We extensively tested our tool, including its prompts, using a unit testing framework.
Our unit tests covered the general processing of manuscript content (such as splitting by paragraphs), the generation of custom prompts using the manuscript metadata, and writing back the text suggestions (ensuring that the original style was preserved as much as possible to minimize the number of changes).
More importantly, they also covered some basic quality measures of the revised text.
This latter set of unit tests was used during our prompt engineering work, and they ensured that section-specific prompts yielded revisions with a minimum set of quality measures.
...
For the introduction section, we checked that a certain percentage of citations were kept, which also attempted to give the model some flexibility to remove text deemed unnecessary.
...
For this particular case, we needed to modify our prompt to explicitly mention the correct format of multiline equations (see the prompt for methods in Figure @fig:ai_revision).
...
Following our criteria for human assessments (see above), we inspected the PRs (Pull Requests) generated by the AI-based workflow and reported our assessment of the changes suggested by the tool across different sections of the manuscripts.
The reader can access the PRs in the manuscripts' GitHub repositories (Table @tbl:manuscripts) and also find them included as diff files in Supplementary File 1 (CCC), 2 (PhenoPLIER), and 3 (Manubot-AI).
...
Red indicates words removed by the tool, while green indicates words added; no underlining indicates words that were kept unchanged.
In GitHub repositories, the full diffs can be seen by clicking on the "Files changed" tab under each PR.
...
It also omitted details about the method (line 5), focusing instead on the aims and results obtained, and concluded with the same last sentence.
This suggests a broader application of the coefficient to other data domains, as originally intended by the authors of CCC.
The main concepts remained present in the revised text.
...
All references to scientific literature were maintained in the correct Manubot format, even though our prompts did not specify the references' format.
...
However, the model failed to maintain the correct format of citations in one paragraph.
Additionally, the model did not generate a revised text for the last paragraph, and our tool left an error message as an HTML comment at the beginning: `The AI model returned an empty string`.
Debugging the prompts revealed this issue, which could be related to the complexity of the paragraph.
In such cases, rerunning the automated revision might resolve this type of issue.
...
The revised paragraph also retains all citations, which, although not explicitly mentioned in the prompts for this section (as it is for introductions), are important in this case.
...
This often resulted in the removal of important details and occasionally altered the intended meaning of sentences.
...
In other instances, the model's output demonstrated the limitations of revising one paragraph at a time without considering the rest of the text.
...
Instead, the new text describes a non-existent experiment with a reference to a non-existent section.
...
For instance, it included the idea of "gene co-expression" analysis (a keyword) to identify "therapeutic targets" (another keyword) and replaced the mention of "sets of genes" in the original text with "clusters of genes" (closer to the keyword "clustering").
This resulted in a poor model-based revision, indicating that the original paragraph might be too short or disconnected from the rest and could benefit from merging with the next one, which describes follow-up and related experiments.
...
One paragraph from the CCC discusses how non-linear correlation coefficients could potentially impact genetic studies of complex traits (Figure @fig:discussion:ccc).
...
For instance, the two articles referenced in lines 2 and 3 of the original text were correctly merged into a single citation block and separated by a ";" in line 2 of the revised text.
...
These details were presented in the following paragraphs.
However, since the model had not yet encountered that information, it opted to add those equations immediately, and in the correct Manubot/Markdown format.
...
When revising the Methods section of Manubot-AI (this manuscript), the model, in some cases, added novel sentences containing incorrect information.
For example, in one paragraph, it included a formula (using the correct Manubot format) presumably to predict the cost of a revision run.
...
It is observed that the two models used as evaluators, GPT-3.5 Turbo and GPT-4 Turbo, generally concurred and favored the revised paragraphs over the original ones (revision score above zero) in most instances.
The sole exception was the Abstract section of the PhenoPLIER and Epistasis manuscripts, where the original paragraphs were distinctly preferred.
Specifically, GPT-3.5 Turbo showed a preference for the original abstract of PhenoPLIER in most cases, and the model's rationale (Supplementary File 5) aligns with our human assessment: the original abstract provides a "detailed explanation" of the methods and a "comprehensive overview of the research."
