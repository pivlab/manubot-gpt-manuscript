...
Finally, by incorporating two external manuscripts (BioChatter and Epistasis), we used the LLM-as-a-Judge technique [@arxiv:2306.05685], where we asked an LLM to evaluate the quality of the revisions produced by another LLM.
...
The paragraph is already well-written and the Markdown formatting has been correctly applied.
However, for clarity and precision, a minor adjustment can be made to ensure consistency in the explanation of the process.
Here is the revised paragraph with the preservation of all Markdown formatting:
...
Following our criteria for human assessments (see above), we inspected the PRs (Pull Requests) generated by the AI-based workflow and reported on our assessment of the changes suggested by the tool across different sections of the manuscripts.
...
These were presented in the following paragraphs, but since the reader had not yet encountered that information, they might expect those equations to be added immediately (in the correct Manubot/Markdown format).
...
It is observable that the two models used as judges, GPT-3.5 Turbo and GPT-4 Turbo, generally concurred and favored the revised paragraphs over the original ones (revision score above zero) in most instances.
The sole section where the original paragraphs were distinctly preferred was the Abstract of the PhenoPLIER and Epistasis manuscripts.
